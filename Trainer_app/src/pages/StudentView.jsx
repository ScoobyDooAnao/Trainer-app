  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        // Busca paralela para acelerar o carregamento
        const [
          { data: cardio },
          { data: plans },
          { data: pr },
          ...
          supabase.from('cardio_sessions').select('*').eq('student_id', studentId).order('date', { ascending: false }),
          supabase.from('workout_plans').select('*').eq('student_id', studentId).eq('status', 'active').order('updated_at', { ascending: false }).limit(1),
          supabase.from('progress_entries').select('*').eq('student_id', studentId).order('date', { ascending: true }),
          supabase.from('attendance').select('date').eq('student_id', studentId).order('date', { ascending: false }),
          supabase.from('exercise_logs').select('*, exercises(name)').eq('student_id', studentId).order('date', { ascending: true }),
        ...
        setLoading(false)  // SEMPRE libera o loading, mesmo em caso de erro
      }
    }
    load()
  }, [studentId])
          ...
          const WeightModal = () => {
            ...
            const save = async () => {
              ...
              const [{ data }] = await Promise.all([
                supabase.from('progress_entries').insert([{ student_id: studentId, date, weight: +peso, notes }]).select().single(),
                supabase.from('students').update({ weight: +peso }).eq('id', studentId),
              ])
              if (data) {
                setProgress(prev => [...prev, data].sort((a,b) => a.date > b.date ? 1 : -1))
                setStudent(prev => ({ ...prev, weight: +peso }))
              }
              setSaving(false)
