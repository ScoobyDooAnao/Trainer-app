...
export default function ParentView({studentId}){
  ...
  useEffect(()=>{
    const load=async()=>{
      try{
        const {data:st,error:e}=await supabase.from('students_public')
          .select('id,name,age,weight,height,goal,level,sport,sport_position,experience_years,guardian_name,teacher_id,birth_date,parent_message,parent_message_date')
