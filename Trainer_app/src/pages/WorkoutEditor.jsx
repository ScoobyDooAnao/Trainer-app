...
const EXERCISE_BANK = [
  ...
  { name: 'Afundo com Rotação',           type: 'Full Body',   sets: '3', reps: '10-12',  rest: '60s',  tip: 'Rotação de tronco no passo' },
  ...
  { name: 'Equilíbrio Unipodal',         type: 'Core',        sets: '3', reps: '20-30s', rest: '30s',  tip: 'Olhos abertos depois fechados para progredir' },
  { name: 'Gato-Vaca (Mobilidade)',      type: 'Core',        sets: '2', reps: '10-15',  rest: '30s',  tip: 'Mobilidade torácica e lombar' },
  { name: 'Mobilidade de Quadril',       type: 'Full Body',   sets: '2', reps: '8-10',   rest: '30s',  tip: '90/90, mobilidade funcional' },
]

// ── Templates ─────────────────────────────────────────────────────────────────
const T = {
  futebol_crianca: {
    label: 'Futebol - FUNdamentals (6-12 anos)', color: '#34D399',
    days: [
  ...
  futebol_adolescente: {
    label: 'Futebol - Train to Train (12-17 anos)', color: '#60A5FA',
    days: [
  ...
  saude: {
    label: 'Saúde e Bem-Estar - Funcional', color: '#34D399',
    days: [
      ...
      { name: 'Cardio + Mobilidade', focus: 'Saúde Cardiovascular', day_of_week: 'Sex', exercises: [
        ...
        { name: 'Equilíbrio Unipodal',  type: 'Core',   sets: '3', reps: '25s',   rest: '30s', tip: 'Prevenção de quedas' },
      ]},
    ],
  },
  massa: {
    label: 'Ganho de Massa - Hipertrofia', color: '#A78BFA',
    days: [
  ...
  forca: {
    label: 'Força e Performance', color: '#EF4444',
    days: [
  ...
  condicionamento: {
    label: 'Condicionamento Físico', color: '#FBBF24',
    days: [
...
const getSuggestedTypes = (goal, sport) => {
  const s = sport || ''
  if (s === 'futebol' || s === 'futsal') return ['Posterior', 'Glúteo', 'Quadríceps', 'Core', 'Full Body']
  if (s === 'natacao')   return ['Costas', 'Ombro', 'Core', 'Full Body']
  if (s === 'basquete')  return ['Quadríceps', 'Glúteo', 'Core', 'Full Body']
          ...
          {ZONES.map(zone => {
            ...
            return (
              <div key={zone.label} style={{ display: 'flex', alignItems: 'center', gap: 8, background: blocked ? 'rgba(255,255,255,0.02)' : zone.color + '10', border: '1px solid ' + (blocked ? 'rgba(255,255,255,0.05)' : zone.color + '30'), borderRadius: 8, padding: '7px 10px', opacity: blocked ? 0.4 : 1 }}>
                <div style={{ flex: 1 }}>
                  <div style={{ fontSize: 11, fontWeight: 700, color: blocked ? '#334155' : zone.color }}>
                    {blocked ? 'Restrito - ' : ''}{zone.label}
                  </div>
                ...
                {load && !blocked ? (
                  <div style={{ textAlign: 'right' }}>
                    ...
                    <button onClick={() => onApply({ reps: zone.reps, rest: zone.rest })} style={{ fontSize: 9, background: zone.color + '20', border: '1px solid ' + zone.color + '40', borderRadius: 5, padding: '2px 7px', color: zone.color, cursor: 'pointer', fontWeight: 700 }}>
                      Usar
                    </button>
                  </div>
                ) : (
  ...
  return (
    <div style={{ position: 'relative', marginBottom: 8 }}>
      <div style={{ display: 'flex', gap: 5, flexWrap: 'wrap', marginBottom: 6 }}>
        ...
        {filterType && (
          <button onClick={() => setFilterType('')} style={{ padding: '3px 8px', borderRadius: 20, border: '1px solid rgba(255,255,255,0.06)', background: 'transparent', color: '#334155', fontSize: 10, cursor: 'pointer' }}>
            x Limpar
          </button>
        )}
      </div>

      <input style={s.smallInput} value={query} onChange={e => setQuery(e.target.value)} placeholder="Buscar exercício... (ex: agachamento, remada, prancha)" />
      ...
      {(query || filterType) && results.length === 0 && (
        <div style={{ padding: '8px 12px', fontSize: 12, color: '#334155', marginTop: 4 }}>Nenhum resultado - preencha manualmente</div>
      )}
    </div>
  )
}
                  ...
                  {day.exercises.map(ex => (
                    <div key={ex.id}>
                      <div style={s.exRow}>
                        ...
                        <select style={{ ...s.smallSel, width: 100, flexShrink: 0 }} value={ex.type || ''} onChange={e => updateExercise(day.id, ex.id, 'type', e.target.value)}>
