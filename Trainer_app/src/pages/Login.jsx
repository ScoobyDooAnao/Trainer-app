...
export default function Login({ onLogin }) {
  ...
  const handle = async () => {
    ...
    if (!supabaseUrl || !supabaseKey) {
      setError('Variáveis de ambiente não encontradas. Verifique o Vercel.')
      return
    }
    setLoading(true)
    setError('')
    ...
    try {
      ...
      if (error) {
        ...
        setWarn('Conta criada! Confirme seu email antes de entrar. Verifique sua caixa de entrada.')
      } else {
        onLogin()
      }
    } catch (e) {
  ...
  return (
    <div style={s.wrap}>
      <div style={s.card}>
        ...
        <div style={s.debug}>
          URL: {supabaseUrl ? '✅ ' + supabaseUrl.slice(0, 30) + '...' : '❌ NÃO ENCONTRADA'}<br />
          KEY: {supabaseKey ? '✅ carregada' : '❌ NÃO ENCONTRADA'}
        </div>

        {error && <div style={s.error}>❌ {error}</div>}
        {warn && <div style={s.warn}>⚠️ {warn}</div>}
        ...
        <div style={{ textAlign: 'center', marginTop: 20, fontSize: 13, color: '#475569' }}>
          {mode === 'login' ? 'Não tem conta? ' : 'Já tem
