...
function imc(w, h) { return h > 0 ? (w / ((h / 100) ** 2)).toFixed(1) : "—"; }
      ...
      <div style={{ position:"relative", zIndex:1, padding:"28px 0", display:"flex", flexDirection:"column", minHeight:"100vh" }}>
        ...
        <div style={{ padding:"0 20px 24px", borderBottom:"1px solid rgba(255,255,255,0.08)", marginBottom:16 }}>
          <div style={{ display:"flex", alignItems:"center", gap:10 }}>
            <div style={{ width:40, height:40, borderRadius:12, background:`linear-gradient(135deg,${C.mustardMid},${C.mustard})`, display:"flex", alignItems:"center", justifyContent:"center", fontSize:20, boxShadow:`0 4px 14px ${C.mustard}55` }}>💪</div>
            <div>
...
function TabAlunos({ students, loading, navigate }) {
  ...
  if (loading) return <div style={{ padding:40, textAlign:"center", color:C.textSub }}>Carregando alunos...</div>;
    ...
    <div>
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:20 }}>
        ...
        <button onClick={() => navigate("/students/new")} style={{
          background:C.navy, border:"none", borderRadius:12, padding:"11px 20px",
      ...
      <div style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:14, marginBottom:28 }}>
        {[
          { label:"Total de Alunos", value: students.length,
            icon:"👥", color:C.navy,    bg:C.navyLight    },
          { label:"Treinos Ativos",  value: students.filter(s=>s.plan_status==="Ativo").length,
            icon:"✅", color:C.emerald, bg:C.emeraldLight },
          { label:"Emagrecimento",   value: students.filter(s=>s.goal==="Emagrecimento").length,
            icon:"🔥", color:C.mustard, bg:C.mustardLight },
          { label:"Ganho de Massa",  value: students.filter(s=>s.goal==="Ganho de Massa").length,
            icon:"💪", color:C.navy,    bg:C.navyLight    },
        ].map((s,i) => (
      ...
      {students.length === 0 ? (
        ...
        <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill,minmax(260px,1fr))", gap:14 }}>
          {students.map(st => {
            ...
            const lastWeight = prog.length > 0 ? prog[prog.length-1].weight : st.weight;
            const lastImc    = lastWeight && st.height ? imc(lastWeight, st.height) : imc(st.weight, st.height);
            const dW = delta(prog, "weight");
            const dI = prog.length > 1 ? +(imc(prog[prog.length-1].weight, st.height) - imc(prog[prog.length-2].weight, st.height)).toFixed(1) : null;
            ...
            return (
              <div key={st.id}
                ...
                style={{
                  ...
                  boxShadow: hovered===st.id ? `0 8px 28px ${C.navy}22` : "0 2px 12px rgba(0,0,0,0.06)",
                  border: hovered===st.id ? `1.5px solid ${C.navyLight}` : `1.5px solid ${C.border}`,
                  cursor:"pointer", transition:"all 0.22s",
                  transform: hovered===st.id ? "translateY(-3px)" : "none",
                }}>
                <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:12 }}>
                  <div style={{ width:44, height:44, borderRadius:13, background:`${goalColor[st.goal] || C.navy}18`, display:"flex", alignItems:"center", justifyContent:"center", fontSize:21 }}>
                    {goalIcon[st.goal] || "🏋️"}
                  </div>
                  <span style={{ fontSize:10, padding:"3px 10px", borderRadius:20, fontWeight:700, background:planBg[st.plan_status] || C.navyLight, color:planColor[st.plan_status] || C.navy }}>
                    {st.plan_status || "Sem treino"}
                  </span>
                </div>
                <div style={{ fontSize:15, fontWeight:700, color:C.text, marginBottom:2 }}>{st.name}</div>
                <div style={{ fontSize:11, color:goalColor[st.goal] || C.navy, fontWeight:600, marginBottom:12 }}>
                  {st.goal || "—"} · {st.level || "—"}
                </div>
                ...
                <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:6 }}>
                  {[["Peso",`${lastWeight}kg`],["IMC", lastImc],["Nivel", st.level?.slice(0,5) || "—"]].map(([l,v]) => (
          ...
          {/* Add card */}
          <div onClick={() => navigate("/students/new")} style={{
            background:"transparent", borderRadius:20, padding:20,
            border:`2px dashed ${C.border}`,
            display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center",
            cursor:"pointer", minHeight:180, gap:8, transition:"all 0.2s",
          }}
...
function TabTreinos({ students, workoutDays, loading }) {
  ...
  // Map workout days per student: { studentId: { "Seg": "Treino A", ... } }
  const planMap = {};
  workoutDays.forEach(wd => {
    if (!planMap[wd.student_id]) planMap[wd.student_id] = {};
    planMap[wd.student_id][wd.day_name] = wd.plan_name;
  });

  if (loading) return <div style={{ padding:40, textAlign:"center", color:C.textSub }}>Carregando treinos...</div>;
  ...
  return (
    <div>
      ...
      <div style={{ display:"flex", gap:8, marginBottom:24, flexWrap:"wrap" }}>
        {allDays.map(d => (
          <div key={d} style={{
            flex:1, minWidth:60, padding:"10px 6px", borderRadius:12, textAlign:"center",
            background: d===todayKey ? C.navy : C.white,
            border: d===todayKey ? `2px solid ${C.navy}` : `1px solid ${C.border}`,
            boxShadow: d===todayKey ? `0 4px 14px ${C.navy}33` : "none",
          }}>
            <div style={{ fontSize:11, fontWeight:800, color: d===todayKey ? C.mustardMid : C.textSub, letterSpacing:1 }}>{d}</div>
      ...
      <div style={{ marginBottom:12 }}>
        <div style={{ fontSize:13, fontWeight:700, color:C.text, marginBottom:14 }}>
          O que cada aluno treina hoje ({todayKey}):
        </div>
        {students.length === 0 ? (
          <div style={{ padding:20, textAlign:"center", color:C.textSub, fontSize:13 }}>Nenhum aluno cadastrado.</div>
        ) : (
          <div style={{ display:"grid", gap:12 }}>
            {students.map(st => {
              const todayWorkout = planMap[st.id]?.[todayKey];
              const hasWorkout   = todayWorkout && todayWorkout !== "Descanso";
              return (
                <div key={st.id} style={{
                  ...
                  <div style={{ width:46, height:46, borderRadius:13, background:`${goalColor[st.goal] || C.navy}18`, display:"flex", alignItems:"center", justifyContent:"center", fontSize:22, flexShrink:0 }}>
                    {goalIcon[st.goal] || "🏋️"}
                  </div>
                  <div style={{ flex:1 }}>
                    ...
                    <span style={{
                      ...
                      color: hasWorkout ? C.navy : C.textSub,
                      background: hasWorkout ? C.navyLight : C.bg,
                      padding:"4px 12px", borderRadius:20,
                      border:`1px solid ${hasWorkout ? C.navyLight : C.border}`,
                    }}>
                      {hasWorkout ? `🏋️ ${todayWorkout}` : "😴 Dia de Descanso"}
                    </span>
                  </div>
                  <span style={{ fontSize:10, padding:"3px 10px", borderRadius:20, fontWeight:700, background:planBg[st.plan_status] || C.navyLight, color:planColor[st.plan_status] || C.navy }}>
                    {st.plan_status || "—"}
        ...
        <div style={{ marginTop:28 }}>
          ...
          <div style={{ background:C.white, borderRadius:16, overflow:"hidden", border:`1px solid ${C.border}` }}>
            <div style={{ display:"grid", gridTemplateColumns:"1.5fr repeat(7,1fr)", background:C.navy }}>
              ...
              {allDays.map(d => (
                <div key={d} style={{ padding:"10px 8px", fontSize:11, fontWeight:700, color: d===todayKey ? C.mustardMid : "rgba(255,255,255,0.6)", textAlign:"center" }}>{d}</div>
              ))}
            </div>
            {students.map((st,i) => (
              <div key={st.id} style={{ display:"grid", gridTemplateColumns:"1.5fr repeat(7,1fr)", borderTop:`1px solid ${C.border}`, background: i%2===0 ? C.white : C.bg }}>
                <div style={{ padding:"12px 16px", fontSize:12, fontWeight:700, color:C.text, display:"flex", alignItems:"center", gap:6 }}>
                  <span>{goalIcon[st.goal] || "🏋️"}</span>
                  {st.name.split(" ")[0]}
                </div>
                {allDays.map(d => {
                  const w       = planMap[st.id]?.[d] || "—";
                  const isRest  = w === "Descanso" || w === "—";
                  const isToday = d === todayKey;
                  return (
                    <div key={d} style={{
                      padding:"10px 6px", fontSize:10, textAlign:"center",
                      color: isRest ? C.border : isToday ? C.navy : C.textSub,
                      fontWeight: isToday ? 700 : 400,
                      background: isToday ? C.navyLight : "transparent",
                    }}>
                      {isRest ? "—" : w.split("—")[0]?.trim() || w}
                    </div>
                  );
                })}
              </div>
...
function TabEvolucao({ students, loading }) {
  ...
  if (loading) return <div style={{ padding:40, textAlign:"center", color:C.textSub }}>Carregando dados...</div>;
  ...
  const prog = (st.progress || []).map(p => ({
    ...p,
    imc: p.weight && st.height ? parseFloat(imc(p.weight, st.height)) : null,
    date: p.measured_at ? new Date(p.measured_at).toLocaleDateString("pt-BR",{month:"short",year:"2-digit"}) : p.date,
  }));
  const isGain = st.goal === "Ganho de Massa";
  ...
  return (
    <div>
      ...
      <div style={{ display:"flex", gap:8, marginBottom:20, flexWrap:"wrap" }}>
        {students.map(s => (
          <button key={s.id} onClick={() => setSelectedId(s.id)} style={{
            ...
            background: selectedId===s.id ? C.navy : C.white,
            color: selectedId===s.id ? "#fff" : C.textSub,
            fontWeight:700, fontSize:12,
            border:`1px solid ${selectedId===s.id ? C.navy : C.border}`,
            boxShadow: selectedId===s.id ? `0 4px 12px ${C.navy}33` : "none",
          }}>
      ...
      <div style={{ background:C.white, borderRadius:16, padding:"16px 20px", border:`1px solid ${C.border}`, marginBottom:20, display:"flex", alignItems:"center", gap:14, flexWrap:"wrap" }}>
        <div style={{ width:48, height:48, borderRadius:14, background:`${goalColor[st.goal] || C.navy}18`, display:"flex", alignItems:"center", justifyContent:"center", fontSize:24 }}>
          {goalIcon[st.goal] || "🏋️"}
        </div>
        <div>
        ...
        <div style={{ marginLeft:"auto", display:"flex", gap:10, flexWrap:"wrap" }}>
          {[
            ...
            ["IMC",        prog.length > 0 ? prog[prog.length-1].imc : imc(st.weight, st.height)],
      ...
      {prog.length < 2 ? (
        ...
        <>
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:16, marginBottom:16 }}>
            {chartCard("⚖️ Peso (kg)", isGain ? "Meta: ganhar massa" : "Meta: reduzir peso",
            ...
            {chartCard("📊 IMC", "Indice de massa corporal",
              <ResponsiveContainer width="100%" height={160}>
                <LineChart data={prog}>
                  ...
                  <YAxis tick={{ fontSize:11, fill:C.textSub }} domain={["auto","auto"]} />
                  <Tooltip contentStyle={{ borderRadius:10, border:`1px solid ${C.border}`, fontSize:12 }} />
                  <Line type="monotone" dataKey="imc" stroke={C.mustard} strokeWidth={2.5} dot={{ fill:C.mustard, r:4 }} name="IMC" />
                </LineChart>
              </ResponsiveContainer>
            )}
          </div>
            ...
            {chartCard("📏 Medidas Corporais (cm)", "Cintura · Peito · Coxa",
              <ResponsiveContainer width="100%" height={160}>
                <LineChart data={prog}>
                  ...
                  <YAxis tick={{ fontSize:11, fill:C.textSub }} domain={["auto","auto"]} />
                  <Tooltip contentStyle={{ borderRadius:10, border:`1px solid ${C.border}`, fontSize:12 }} />
                  <Line type="monotone" dataKey="waist"  stroke={C.emerald} strokeWidth={2} dot={{ fill:C.emerald, r:3 }} name="Cintura" />
                  <Line type="monotone" dataKey="chest"  stroke={C.navy}    strokeWidth={2} dot={{ fill:C.navy,    r:3 }} name="Peito"   />
                  <Line type="monotone" dataKey="thigh"  stroke={C.mustard} strokeWidth={2} dot={{ fill:C.mustard, r:3 }} name="Coxa"    />
            ...
            {chartCard("🏋️ Carga nos Exercicios (kg)", "Progressao de forca",
              <ResponsiveContainer width="100%" height={160}>
                <LineChart data={prog}>
                  ...
                  <YAxis tick={{ fontSize:11, fill:C.textSub }} domain={["auto","auto"]} />
                  <Tooltip contentStyle={{ borderRadius:10, border:`1px solid ${C.border}`, fontSize:12 }} />
                  <Line type="monotone" dataKey="max_load" stroke="#7C3AED" strokeWidth={2.5} dot={{ fill:"#7C3AED", r:4 }} name="Carga (kg)" />
                </LineChart>
              </ResponsiveContainer>
            )}
          </div>
...
export default function Dashboard() {
  ...
  useEffect(() => {
    async function loadAll() {
      ...
      const studentsWithProgress = await Promise.all(studs.map(async st => {
        ...
        return { ...st, progress: prog || [], plan_status: plan?.status || null };
      }));

      // Busca dias de treino de todos os alunos
      const { data: wDays } = await supabase
        .from("workout_days")
        .select("id, plan_id, day_name, workout_plans(student_id, status)")
        .order("id");

      const flatDays = (wDays || []).map(d => ({
        ...d,
        student_id: d.workout_plans?.student_id,
        plan_name: d.day_name,
      }));
